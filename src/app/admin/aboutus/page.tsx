...
export default function AdminAboutUsPage() {
  ...
  const [aboutUsExists, setAboutUsExists] = useState(false);
  ...
  // Завантаження даних About Us секції
  const loadAboutUs = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("authToken");
      const res = await fetch(`${BACKEND_URL}/api/v1/aboutus`, {
      ...
      if (res.ok) {
        ...
        throw new Error("Не вдалося отримати About Us");
      }
    } catch (e) {
      console.error(e);
      showError("Помилка завантаження About Us");
      setAboutUsExists(false);
    } finally {
      setLoading(false);
    }
  ...
  // Збереження даних About Us секції
  const saveAboutUsData = async (imageUrl?: string) => {
    try {
      const token = localStorage.getItem("authToken");
      const method = aboutUsExists ? "PUT" : "POST";
      ...
      showSuccess(
        aboutUsExists
          ? "About Us оновлено успішно"
          : "About Us створено успішно"
      );
    } catch (e: any) {
  ...
  const uploadPhoto = async (file: File) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/v1/aboutus/upload`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                value={aboutUs.title}
                onChange={(e) =>
                  setAboutUs((a) => ({ ...a, title: e.target.value }))
                }
                className="w-full border rounded-md px-3 py-2 text-black"
                placeholder="Введіть заголовок секції About Us"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setAboutUs((a) => ({ ...a, description: e.target.value }))
                }
                className="w-full border rounded-md px-3 py-2 text-black h-32"
                placeholder="Введіть детальний опис секції About Us"
              />
            </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="mb-4">
            <input
              ...
              disabled={photo.uploading || aboutUsExists}
            />
          </div>
          ...
          <button
            ...
            disabled={!photo.file || photo.uploading || aboutUsExists}
            className={`w-full py-2 px-4 rounded-md font-medium ${
              photo.file && !photo.uploading && !aboutUsExists
        ...
        <div className="mt-8 p-4 bg-green-50 rounded-lg">
          ...
          <ul className="text-green-800 space-y-1">
            ...
            <li>• Кожна секція About Us може мати тільки одне зображення</li>
            <li>
