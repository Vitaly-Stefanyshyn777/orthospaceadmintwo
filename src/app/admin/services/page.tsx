...
export default function ServicesPage() {
  ...
  // Автоматичне заповнення тестовими даними
  const fillWithTestData = async () => {
    if (!confirm("Це створить всі тестові категорії та послуги. Продовжити?")) {
      return;
    }
    ...
    try {
      ...
      showSuccess("🔄 Початок заповнення тестовими даними...");

      for (const categoryData of TEST_DATA) {
        console.log(`📁 Створення категорії: ${categoryData.mainTitle}`);
        ...
        const categoryResponse = await fetch(
          `${BACKEND_URL}/api/v1/services/categories`,
          {
            ...
            body: JSON.stringify({
              ...
              order: parseInt(categoryData.categoryId), // Використовуємо categoryId як order
            }),
          }
        );
        ...
        if (!categoryResponse.ok) {
          ...
          console.error(
            `❌ Помилка створення категорії ${categoryData.mainTitle}:`,
            errorData
          );
          continue;
        }
        ...
        console.log(`✅ Створено категорію: ${createdCategory.mainTitle}`);
        createdCategories++;

        // Створюємо послуги для цієї категорії
        for (const serviceData of categoryData.services) {
          console.log(`💼 Створення послуги: ${serviceData.name}`);
          ...
          const serviceResponse = await fetch(
            `${BACKEND_URL}/api/v1/services`,
            {
              ...
              body: JSON.stringify({
                ...
                isActive: true,
                order: createdServices % 10, // Простий порядок
          ...
          if (!serviceResponse.ok) {
            ...
            console.error(
              `❌ Помилка створення послуги ${serviceData.name}:`,
              errorData
            );
          } else {
            console.log(`✅ Створено послугу: ${serviceData.name}`);
            createdServices++;
          }
      ...
      await fetchCategories();
      showSuccess(
        `🎉 Успішно створено ${createdCategories} категорій та ${createdServices} послуг!`
      );
    } catch (err) {
      console.error("❌ Помилка заповнення тестовими даними:", err);
      showError(
        `Помилка заповнення: ${
          err instanceof Error ? err.message : "Невідома помилка"
        }`
